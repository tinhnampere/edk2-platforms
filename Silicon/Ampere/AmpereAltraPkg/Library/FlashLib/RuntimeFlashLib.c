...
extern BOOLEAN                        mIsEfiRuntime;
extern UINT8                          *mTempBufferPhysical;
extern UINT8                          *mTempBufferVirtual;
...
{
  gRT->ConvertPointer (0x0, (VOID **)&mTempBufferVirtual);
  ...
  )
{
  ...
  mTempBufferPhysical = AllocateRuntimeZeroPool (EFI_MM_MAX_TMP_BUF_SIZE);
  mTempBufferVirtual = mTempBufferPhysical;
  ASSERT (mTempBufferPhysical != NULL);
