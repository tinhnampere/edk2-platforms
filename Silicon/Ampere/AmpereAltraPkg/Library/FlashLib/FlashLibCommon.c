...
BOOLEAN                       mIsEfiRuntime = FALSE;
UINT8                         *mTempBufferPhysical;
  ...
  )
{
  if (mIsEfiRuntime) {
    ...
    CopyMem (mTempBufferVirtual, VirtualPtr, Size);
    return mTempBufferPhysical;
  }
  ...
  )
{
  ...
  while (Remain > 0) {
    ...
    MmData[1] = ByteAddress + Count;
    MmData[2] = NumRead;
    MmData[3] = (UINT64)mTempBufferPhysical;  // Read data into the temp buffer with specified virtual address
    ...
    //
    CopyMem ((VOID *)(Buffer + Count), (VOID *)mTempBufferVirtual, NumRead);
    Remain -= NumRead;
    Count += NumRead;
  }
