...
STATIC EFI_ACPI_SERVICE_PROCESSOR_MANAGEMENT_INTERFACE_TABLE mSpmiTable = {
  ...
  0,                        // Global System Interrupt
  {
    ...
    0,                      // Address (7-bit SMBUS Address of BMC SSIF)
  },
  {
    {
      0,                    // UID Byte 1
...
{
  SpmiTable->BaseAddress.Address = FixedPcdGet8 (PcdBmcSlaveAddr);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  Status = UpdateSmbusAddress (&mSpmiTable);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to update SMBUS Address - %r \n", __FUNCTION__, Status));
    return Status;
  }
